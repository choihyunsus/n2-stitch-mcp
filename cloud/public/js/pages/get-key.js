...
function _initForm() {
  ...
  // Auto-select plan from pricing card CTA (passed via BillingPeriod)
  const passedPlan = BillingPeriod.consumePlan();
  if (passedPlan && CONFIG.PLANS[passedPlan]) {
    selectedPlan = passedPlan;
    planSelector?.querySelectorAll('.plan-option').forEach(option => {
      option.classList.toggle('selected', option.dataset.plan === passedPlan);
    });
  }
